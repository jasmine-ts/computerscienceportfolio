...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if isinstance(place.ant, ContainerAnt) and (place.ant.contained_ant == None) and not isinstance(self, ContainerAnt): #ant currently in place is a container ant
                place.ant.contain_ant(self)
            elif isinstance(self, ContainerAnt) and (self.contained_ant == None) and not isinstance(place.ant, ContainerAnt): #ant being added is a container ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        range_count = 0
        location = ant_location
        while location != beehive:
            if random_or_none(location.bees):
                if self.min_range <= range_count <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        bee_list = ant_location.bees
        bee_list_copy = bee_list[:]
        [Insect.reduce_armor(bee, amount) for bee in bee_list_copy]

        Insect.reduce_armor(self, amount)

        if self.armor == 0:
            bee_list_copy = bee_list[:]
            [Insect.reduce_armor(bee, bonus_damage) for bee in bee_list_copy]
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.digesting != 0:
            ...
            ant_location = self.place
            eat_this_bee = random_or_none(ant_location.bees)
            self.eat_bee(eat_this_bee)
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        bee_list_copy = bee_list[:]
        [Insect.reduce_armor(bee, self.damage) for bee in bee_list_copy]
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        if not other.contained_ant or (other is not ContainerAnt):
            return True
        else:
            return False
...
class QueenAnt(ScubaThrower):  
    ...
    name = 'Queen'
    food_cost = 7
    imposters_exist = False
    true_queen = False
    implemented = True   # Change to True to view in the GUI

    def __init__(self, armor=1):
        ScubaThrower.__init__(self, armor)
        if QueenAnt.imposters_exist == False:
            self.true_queen = True
            QueenAnt.imposters_exist = True
            ...
            while place:
                if place.ant:
                    ...
                    if isinstance(place.ant, ContainerAnt) and place.ant.contained_ant:
                        the_contained_ant = [place.ant.contained_ant]
                    else:
                        the_contained_ant = []
                    ants_behind_queen = ants_behind_queen + ant_in_place + the_contained_ant
                place = place.exit
...
class Bee(Insect):
    ...
    damage = 1
    is_watersafe = True
    direction = True #True if forward, False if backwards
    scared_before = False
    ...
    def blocked(self):
        ...
        if self.place.ant and not self.place.ant.blocks_path:
            return False
        elif self.place.ant and self.place.ant.blocks_path:
            return True
        else:
            return False
...
def apply_effect(effect, bee, duration):
    ...
    def effect_applied(gamestate):
        nonlocal turns_count
        if turns_count < duration:
            turns_count += 1
            return modified_action(gamestate)
        if effect is make_scare:
            bee.direction = True
        return bee_action(gamestate)
